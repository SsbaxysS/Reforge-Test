        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)', background: 'var(--bg-card)' }}>
          <div className="p-5 sm:p-8">
            <div className="mb-6">
              ...
              <h1 className="text-lg font-bold mb-1" style={{ color: 'var(--text-100)' }}>Создать аккаунт</h1>
              <p className="text-sm" style={{ color: 'var(--text-500)' }}>Зарегистрируйтесь за минуту</p>
            </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={firstName} onChange={e => setFirstName(e.target.value)} required placeholder="Иван"
                      className="w-full px-3.5 py-2.5 rounded-xl text-sm focus:outline-none transition-colors" style={inputStyle} />
                  </div>
                  <div>
                    <label className="block text-xs mb-1.5" style={{ color: 'var(--text-500)' }}>Фамилия</label>
                    <input type="text" value={lastName} onChange={e => setLastName(e.target.value)} required placeholder="Иванов"
                      className="w-full px-3.5 py-2.5 rounded-xl text-sm focus:outline-none transition-colors" style={inputStyle} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="Минимум 6 символов"
                      className="w-full px-3.5 py-2.5 pr-10 rounded-xl text-sm focus:outline-none transition-colors" style={inputStyle} />
                    <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 transition-colors" style={{ color: 'var(--text-600)' }}>
                ...
                <div>
                  <label className="block text-xs mb-1.5" style={{ color: 'var(--text-500)' }}>Подтвердите пароль</label>
            ...
            <p className="text-center text-sm mt-6" style={{ color: 'var(--text-600)' }}>
              Уже есть аккаунт?{' '}
              <Link to="/login" className="transition-colors" style={{ color: 'var(--accent-light)' }}>Войти</Link>
            </p>
          </div>
        </div>
      </div>
